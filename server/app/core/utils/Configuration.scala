...
class Configuration(val configuration: play.api.Configuration) {
  ...
  def getBoolean(key: String) = Getter[Boolean](key, configuration.getOptional[Boolean](key))

  def getInt(key: String) = Getter[Int](key, configuration.getOptional[Int](key))

  def getLong(key: String) = Getter[Long](key, configuration.getOptional[Long](key))

  def getDouble(key: String) = Getter[Double](key, configuration.getOptional[Double](key))

  def getString(key: String) = Getter[String](key, configuration.getOptional[String](key))
...
object Configuration {
  ...
  def apply(conf: play.api.Configuration): Configuration = new Configuration(conf)
}
